...
def test_overlap_metric_shape_and_symmetry() -> None:
    ...
    matrix = compute_overlap(basis, method=MetricMethod.OVERLAP, n_radial_grid=256)
...
def test_overlap_alias_s_matches_overlap() -> None:
    basis = _make_basis(n_max=2, l_max=1)
    m1 = compute_overlap(basis, method="overlap", n_radial_grid=256)
    m2 = compute_overlap(basis, method="s", n_radial_grid=256)
    np.testing.assert_allclose(m1, m2, atol=1e-12)


def test_coulomb_alias_j_matches_coulomb() -> None:
    basis = _make_basis(n_max=2, l_max=1)
    m1 = compute_overlap(basis, method="coulomb", n_radial_grid=192)
    m2 = compute_overlap(basis, method="j", n_radial_grid=192)
    np.testing.assert_allclose(m1, m2, atol=1e-12)


def test_different_l_or_m_blocks_do_not_couple() -> None:
    basis = _make_basis(n_max=2, l_max=1)
    matrix = compute_overlap(basis, method="overlap", n_radial_grid=192)
...
def test_single_basis_overlap_is_unity_with_reasonable_quadrature() -> None:
    ...
    matrix = compute_overlap(basis, method="overlap", n_radial_grid=512)
